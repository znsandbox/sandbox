...
<a href="<?= \ZnLib\Web\Helpers\Url::to([$baseUri, 'id' => $favoriteEntityItem->getId()]) ?>"
   ...
   class="list-group-item list-group-item-action <?= $isActive ? 'active' : '' ?>">
    <div class="d-flex w-100 justify-content-between">
        ...
        $lifeTimeLimitNew = \ZnCore\Base\Time\Enums\TimeEnum::SECOND_PER_DAY;
        ...
        if($favoriteEntityItem->getUpdatedAt()) {
            ...
            $isUpd = $lifeTimeUpd < \ZnCore\Base\Time\Enums\TimeEnum::SECOND_PER_DAY;
            $colorUpd = '#ffc107';
            if($lifeTimeUpd < \ZnCore\Base\Time\Enums\TimeEnum::SECOND_PER_HOUR * 8 * 1) {
                $colorUpd = '#ffc107';
            } elseif($lifeTimeUpd < \ZnCore\Base\Time\Enums\TimeEnum::SECOND_PER_HOUR * 8 * 2) {
                $colorUpd = 'rgba(255,193,7,0.66)';
            } elseif($lifeTimeUpd < \ZnCore\Base\Time\Enums\TimeEnum::SECOND_PER_HOUR * 8 * 3) {
